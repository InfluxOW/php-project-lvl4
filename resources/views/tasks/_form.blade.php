...
<div class="text-muted">
    ...
    {{ Form::select('status_id', $statuses, $task->status->id ?? $statusNew, ['class' => 'form-control mb-4']) }}
    {{ Form::label('assignees', __('Assignees'), ['class' => 'form-group font-weight-normal my-1']) }}<br>
    {{ Form::select('assignees[]', $users, $task->assignees->pluck('id')->toArray(), ['class' => 'form-control mb-4', 'multiple' => 'multiple'])  }}
    {{ Form::label('labels', __('Labels'), ['class' => 'form-group font-weight-normal my-1']) }}<br>
    {{ Form::select('labels[]', $labels, $task->labels->pluck('id')->toArray(), ['class' => 'form-control mb-4', 'multiple' => 'multiple'])  }}
</div>
